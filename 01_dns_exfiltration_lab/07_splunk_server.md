...
`current\_only = 1` means the forwarder collects events generated after the input starts instead of importing the entire historical channel.
...
| eval event\_time=strftime(\_time,"%Y-%m-%d %H:%M:%S"),
       indexed\_time=strftime(\_indextime,"%Y-%m-%d %H:%M:%S")
| table event\_time indexed\_time host source sourcetype EventCode
| sort 0 -indexed\_time
```
