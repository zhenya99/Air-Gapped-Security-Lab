...
> **Important:** This step assumes the Proxmox host, external Security Onion storage, `vmbr0`, and `vmbr1` have already been configured. The final known-good backbone restricts `vmbr0` to VLANs **10 and 99**, keeps the management path at MTU 1500, and uses **MTU 9000** on the passive capture path (`nic1` → `vmbr1` → VM `net1`). Do not wipe or reformat an existing storage device during a Security Onion reinstall unless you intentionally want to destroy the underlying Proxmox storage pool.
...
nic0     UP    MTU 1500
nic1     UP    MTU 9000
vmbr0    UP    172.16.99.20/24    MTU 1500
vmbr1    UP    no IPv4 address    MTU 9000
```
...
| --------- | ------- | ---------- | ------------------------------- |
| `nic0`    | `vmbr0` | Gi1/0/27   | Proxmox management + VM traffic, MTU 1500 |
| `nic1`    | `vmbr1` | Gi1/0/28   | Passive SPAN capture, MTU 9000            |
...
Both should report `UP` and `LOWER_UP`.

The expected MTUs are different by design:
...
```bash
ip link show nic0 | head -1
ip link show nic1 | head -1
ip link show vmbr1 | head -1
```

The capture bridge `vmbr1` must also report `mtu 9000`.
...
Before installing Security Onion, verify that mirrored traffic from the configured Cisco SPAN source (`Gi1/0/27`, direction `both`) is arriving at the physical capture NIC.
...
## 1.14 Verify VLAN Restrictions and Capture MTU

Verify the live VLAN table with compressed ranges:

```bash
bridge -compressvlans vlan show
```

The physical management uplink should show only the VLANs required by this lab:
...
The dynamically created VM tap interface `tap900i0` may display a broader range; the important enforcement point is the physical uplink `nic0`.

Verify the capture MTU and VM sensor NIC:
...
## 1.15 Optional: Back Up the Existing VM Configuration
...
## 1.16 Final Pre-Installation Validation
...
* [ ] `vmbr0` allows only VLANs `10 99`
* [ ] `nic1`, `vmbr1`, and VM `net1` use MTU `9000`
* [ ] Cisco SPAN source is `Gi1/0/27` in both directions and destination is `Gi1/0/28`
